...
export class PageController extends EventTarget {
	...
	/**
	 * Click element by CSS selector (bypasses index mechanism).
	 * Uses the same W3C standard event simulation as clickElement().
	 */
	...
	/**
	 * Input text into element by CSS selector (bypasses index mechanism).
	 * Uses the same dual-strategy input as inputText() - supports Slate.js, React, etc.
	 */
	async inputBySelector(selector: string, text: string): Promise<ActionResult> {
		try {
			const element = document.querySelector(selector) as HTMLElement
			if (!element) {
				return { success: false, message: `❌ Element not found: ${selector}` }
	...
	/**
	 * Find element by CSS selector and return its current index in the selectorMap.
	 * Useful for mixing stable selectors with index-based operations.
	 */
	async getIndexBySelector(selector: string): Promise<number> {
